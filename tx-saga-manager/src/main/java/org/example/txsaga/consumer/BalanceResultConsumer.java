...
public class BalanceResultConsumer {
    ...
    @KafkaListener(topics = "${kafka.topic.balance-rollback-success}", groupId = "tx-saga-manager", concurrency = "3", containerFactory = "containerFactory")
    public void rollbackResult(ConsumerRecord<String, String> record) {

        log.info("consumed. topic : {}, key : {}, value : {}", record.topic(), record.key(), record.value());

        var payload = CommonUtil.commonMapper.readValue(record.value(), LedgerSucceeded.class);
        sagaProcessor.sendNoti(payload);
